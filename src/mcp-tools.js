// The agent-facing tool set. Registered onto an McpServer by both transports:
//   mcp/server.js   -> stdio, for a local Claude Code / Claude Desktop
//   src/mcp-http.js -> Streamable HTTP at /mcp, for remote connectors
// Five read tools and one write tool that only creates a draft. There is deliberately no approve/send tool.
const { McpServer } = require('@modelcontextprotocol/sdk/server/mcp.js');
const { z } = require('zod');
const config = require('./config');
const jobs = require('./jobs');
const followups = require('./followups');
const { validateFollowUpDraft, validateDate, ValidationError } = require('./validate');
...
const fail = (err) => ({
  isError: true,
  content: [{ type: 'text', text: err.expose || err instanceof ValidationError ? err.message : `Tool failed: ${err.message}` }],
});
...
function registerTools(server) {
  server.registerTool(
    ...
    run(async ({ job_id }) => {
      ...
      return {
        job,
        follow_ups: await followups.listFollowUps({ jobId: job.job_id }),
        pdf_url: job.has_pdf ? `${config.appBaseUrl}/api/jobs/${job.job_id}/pdf` : null,
      };
    })
  );
...
module.exports = { registerTools, createMcpServer };
